...
int CCommandProcessor::DumpCrashReport(LPCWSTR szCrashRptFileName, LPCWSTR szOutFile, LPCWSTR szSymbolSearchDir, bool bExactMatchBuildAge)
{
	...
	if(pSysInfo && !(
		pSysInfo->m_uProcessorArchitecture == PROCESSOR_ARCHITECTURE_INTEL || 
		pSysInfo->m_uProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64 ||
		pSysInfo->m_uProcessorArchitecture == PROCESSOR_ARCHITECTURE_IA64
		))
	{
